---
layout: about
title: home
permalink: /

news: true # includes a list of news items
latest_posts: false # includes a list of the newest posts
selected_papers: false # includes a list of papers marked as "selected={true}"
social: false # includes social icons at the bottom of the page
---

<div class="lang-en" markdown="1">

### About the Lab

We are a cryptography research group in the Department of Convergence Security Engineering
at Sungshin Women's University.
Our research lies at the intersection of **applied cryptography** and **cryptanalysis**, with two main themes:

- **Cryptography for Collaborative Analysis.**
  We design cryptographic protocols that let multiple parties jointly analyze their data
  without revealing it to each other.
  This includes _homomorphic encryption_, _secure multi-party computation_,
  and their applications such as _private set intersection (PSI)_, _private record linkage_,
  and _privacy-preserving machine learning / genomic analysis_.

- **Analysis of Cryptographic Assumptions.**
  We study the concrete security of the hardness assumptions that modern (post-quantum) cryptography is built on,
  most notably _lattice problems_ such as LWE with small or sparse secrets.
  Understanding how hard these problems really are is essential for choosing safe and efficient parameters.

<div class="recruiting-box">
  <h4>📢 We are recruiting!</h4>
  <p>
    We are always looking for motivated students who want to work on cryptography and privacy-enhancing technologies:
    <b>graduate students (M.S. / Ph.D.)</b> and <b>undergraduate interns</b>.
    A background in mathematics, computer science, or security is welcome, and no prior experience in cryptography is required.
  </p>
  <p class="mb-0">
    If you are interested, please send an e-mail to <b>yongha DOT son AT sungshin.ac.kr</b>
    with a short introduction. See the <a href="/people/">People</a> page for the current members.
  </p>
</div>

</div>

<div class="lang-ko" markdown="1">

### 연구실 소개

성신여자대학교 융합보안공학과의 암호학 연구실입니다.
**응용 암호**와 **암호 분석**의 접점에서, 크게 두 가지 주제를 연구합니다.

- **협력적 데이터 분석을 위한 암호 기술.**
  여러 기관이 서로의 데이터를 공개하지 않고도 함께 분석할 수 있게 하는 암호 프로토콜을 설계합니다.
  _동형암호_, _안전한 다자간 계산(MPC)_ 과 이를 응용한 _프라이버시 보존 교집합 연산(PSI)_,
  _안전한 레코드 결합_, _프라이버시 보존 기계학습 및 유전체 분석_ 등이 여기에 포함됩니다.

- **암호학적 가정의 분석.**
  현대 (양자내성) 암호가 기반으로 삼는 어려운 문제들, 특히 작은 비밀키나 희소 비밀키를 갖는 LWE와 같은
  _격자 문제_ 의 구체적인 안전성을 분석합니다.
  이 문제들이 실제로 얼마나 어려운지를 이해하는 것은 안전하면서도 효율적인 파라미터를 고르는 데 필수적입니다.

<div class="recruiting-box">
  <h4>📢 연구실 학생을 모집합니다!</h4>
  <p>
    암호학과 프라이버시 강화 기술에 관심 있는 <b>대학원생(석사 / 박사)</b>과 <b>학부 인턴</b>을 상시 모집합니다.
    수학, 컴퓨터과학, 보안 관련 배경이면 환영하며, 암호학을 미리 공부했을 필요는 없습니다.
  </p>
  <p class="mb-0">
    관심 있는 분은 간단한 자기소개와 함께 <b>yongha DOT son AT sungshin.ac.kr</b> 로 메일 주세요.
    현재 구성원은 <a href="/people/">People</a> 페이지에서 볼 수 있습니다.
  </p>
</div>

</div>
